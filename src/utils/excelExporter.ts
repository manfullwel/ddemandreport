...
export const exportToExcel = async (
    ...
    const julioEmployees = data.employees.filter(emp => emp.group === 'JULIO').map(emp => emp.name);
    const adrianoEmployees = data.employees.filter(emp => emp.group === 'ADRIANO/LEANDRO').map(emp => emp.name);
